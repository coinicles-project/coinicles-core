...
if len(sys.argv) < 3:
    ...
    if m:
        ...
        if "result" in r and "service_node_x25519_privkey" in r["result"]:
            ...
            print("Loaded x25519 keys (pub: {}) from coiniclesd @ {}".format(hexstr(x_key.public_key), sys.argv[1]))
        else:
            x_key = PrivateKey.generate()
            print("Generated x25519 key {} (coiniclesd @ {} did not return SN privkeys)".format(hexstr(x_key.public_key), sys.argv[1]))
    else:
        print("Error: {} does not look like a valid coiniclesd RPC host:port value".format(sys.argv[1]), sys.stderr)
        badargs = True
