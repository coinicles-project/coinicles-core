...
class Daemon(RPCDaemon):
    ...
    def __init__(self, *,
        ...
        if name is None:
            name = 'coiniclesd@{}'.format(self.rpc_port)
        super().__init__(name)
        ...
        self.args = [coiniclesd] + list(self.__class__.base_args)
        self.args += (
    ...
    def ping(self, *, storage=True, lokinet=True):
        """Sends fake storage server and lokinet pings to the running coiniclesd"""
        if storage:
...
class Wallet(RPCDaemon):
    ...
    def __init__(
            ...
            node,
            *,
            rpc_wallet='coinicles-wallet-rpc',
            name=None,
            datadir=None,
            listen_ip=None,
            rpc_port=None,
